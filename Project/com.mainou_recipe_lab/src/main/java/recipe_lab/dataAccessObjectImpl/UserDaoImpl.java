...
public class UserDaoImpl implements UserDao {
	...
	@Override
	public boolean add(User user) { //method to add user into database
		try {
	...
	public boolean delete(String email) {
		...
		user.setEnabled(false);   //instead of deleting the user data from database we set enabled value to false
		try {
			sessionFactory.getCurrentSession().update(user);
			return true;
		}catch(Exception msg) {
	...
	public User getUserById(Long user_id) {
		try {
			...
			msg.printStackTrace();
			System.out.println("Something error occured during GetUser BY Id");
			return null;
		}
	}
